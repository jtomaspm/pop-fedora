...
verify_nvidia_kernel_module() {
    if modinfo -F version nvidia >/dev/null 2>&1; then
        pf_log_success "The NVIDIA kernel module is available."
        return 0
    fi

    pf_log_warning "The NVIDIA kernel module is not ready yet. Re-run 'modinfo -F version nvidia' before rebooting."
}
...
handle_nvidia_drivers() {
    ...
    install_nvidia_drivers
    pf_log_info "Allow the NVIDIA kernel module time to build before rebooting."
    verify_nvidia_kernel_module
}
